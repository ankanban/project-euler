...
def all_lines(grid):
    cols = len(grid[0])
    rows = len(grid)
    print("%s x %s" % (rows, cols))
    al = []
    # Rows
    for r in xrange(rows):
        al.append(grid[r])
        al.append(list(reversed(grid[r])))
    # Cols
    for c in xrange(cols):
        col = [grid[r][c] for r in xrange(rows)]
        al.append(col)
        al.append(list(reversed(col)))
    # Diags
    for r in xrange(rows):
        for c in xrange(cols):
            def generate_col_diag(dirn=True):
                if dirn:
                    x = c
                    y = r
                    ...
                    while x >= 0 and y < rows:
                        ...
                        x -= 1
                        y += 1
            diag_left = [x for x in generate_col_diag()]
            al.append(diag_left)
            al.append(list(reversed(diag_left)))
            diag_right = [x for x in generate_col_diag(False)]
            al.append(diag_right)
            al.append(list(reversed(diag_right)))
    return al

def find_largest_fp(grid):
    prods = [four[0] * four[1] * four[2] * four[3]
             for line in all_lines(grid) for four in all_fours(line)]
    print(prods)
    largest_fp = 0
    for fp in prods:
        if fp > largest_fp:
...
if __name__ == '__main__':
    ...
    for line in all_lines(GRID):
        for four in all_fours(line):
            fp = four[0] * four[1] * four[2] * four[3]
            print(fp)
            if fp > largest_fp:
                largest_fp = fp
    '''
    fp = find_largest_fp(GRID)
    print(fp)
